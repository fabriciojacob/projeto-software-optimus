...
public class ParticipanteBean extends FacesUtil implements Serializable{
	...
	public void salvarPJ() {
		...
		if (retorno == 0) {
			...
			this.error("Existem campos Nulo no Formul�rio");
		}
	}
	...
	public void salvarPF() {
		try {
			...
			if (retorno == 0) {
				...
				this.error("Existem campos n�o preenchidos");
			}
		} catch (Exception e) {
	...
	public void salvarEmail() {
		...
		if (retorno == 0) {
			try {
				...
				this.error("Problemas na valida��o do email"+ e.getMessage());
			}
			if (this.isPadraoNFE() && checkNFe == 1) {
				...
				try {
					...
					this.error("Problemas na inclus�o do email"+ e.getMessage());
				}
			}
		} else {
